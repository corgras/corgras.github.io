...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (!button) return; // Если кнопка не найдена, завершаем выполнение скрипта
    
    const icon = button.querySelector("i.ics");
    const codeTextElement = document.querySelector(".code-snippet .line");
    if (!icon || !codeTextElement) return; // Если иконка или текст отсутствуют, завершаем выполнение
