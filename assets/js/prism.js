/* PrismJS 1.29.0
https://prismjs.com/download.html#themes=prism&languages=markup+css+clike+javascript+basic+batch+bbcode+css-extras+csv+diff+git+http+hpkp+hsts+icon+ignore+ini+javadoclike+jq+bash+powershell+jsdoc+js-extras+json+json5+js-templates+markdown+markup-templating+metafont+php+phpdoc+php-extras+sass+scss+scheme+sql+twig+typescript+xml-doc&plugins=show-language+autoloader+normalize-whitespace+toolbar+copy-to-clipboard */
...
(function (Prism) {
	...
	Prism.languages.npm = Prism.languages.bash;
}(Prism));
...
(function () {
    ...
    'ru': {
        ...
        'copy-error': 'Нажмите Ctrl+C для копирования',
        'copy-success': 'Скопировано!',
        'copy-timeout': 5000
    },
    'uk': {
        'copy': 'Скопіювати код',
        'copy-error': 'Натисніть Ctrl+C для копіювання',
        'copy-success': 'Скопійовано!',
        'copy-timeout': 5000
    }
};

// Получаем язык из атрибута lang у тега html
var userLang = document.documentElement.lang || 'uk'; // По умолчанию 'en'

// Выбор настроек в зависимости от языка
var settings = translations[userLang] || translations['en']; // Используем английский как запасной вариант
	...
	Prism.plugins.toolbar.registerButton('copy-to-clipboard', function (env) {
		...
		function setState(state) {
			var icons = {
				...
				'copy-success': '<svg stroke="currentColor" fill="none" stroke-width="2" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round" class="icon-sm" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><polyline points="20 6 9 17 4 12"></polyline></svg>'
			};
			linkSpan.innerHTML = icons[state] + settings[state];
